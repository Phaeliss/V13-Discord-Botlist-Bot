...
exports.run = async (client, message, args) => {
  const embed = new Discord.MessageEmbed()
    ...
    if(message.member.roles.cache.has(db.get(`yetkilirol${message.guild.id}`))) return message.channel.send(embed.setDescription(`Lütfen Yetkililere Özel Komutu Kullanın \`yetkili-bot-profil\``))
    let botID = args[0]
     ...
     if(!bot) return message.channel.send(embed.setDescription(`Sistemde **${discordBot.username}** İsimli Botunuzu Bulamadım.`))
     if(message.author.id !== bot.owner){
       return message.reply(`Botun Sahibi Sen Değilsin Botun Bilgilerini Sana Gösteremem`)
     }
...
exports.conf = {
  aliases: ["bot-profile","botprofil","botprofil"],
};
